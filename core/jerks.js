...
export const newJerk = (x, y) => {
  ...
  jerkBody.beginFill(0x000000);
  jerkBody.drawRect(0, 0, 20, 20);
  jerkBody.x = x - 10;
  jerkBody.y = y - 10;
  jerkBody.endFill();
  jerkBody.zIndex = 2;
...
const action_run = (jerk) => {
  if (jerk.hasToxic) {
    jerk.x++;
toxicArray[jerk.toxicCode].x=jerk.x
toxicArray[jerk.toxicCode].y=jerk.y-20
...
export const theJerkWin = (jerk) => {
    ...
    let toxicToKillIndex = aliveToxicArray.findIndex((e) => e.code === jerk.toxicCode);
